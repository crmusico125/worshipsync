...
function buildSlides(lyrics: string, sectionLabel: string, sectionId: number, lineupItemId: number, maxLines = 2): Slide[] {
  ...
  if (slides.length === 0) {
    slides.push({ lines: [''], sectionLabel, sectionId, slideIndex: 0, totalSlides: 1 })
  }
  return slides
}
...
export default function LiveScreen({ onClose, projectionOpen }: Props) {
  ...
  // Navigation
  const goToSlide = useCallback((songIdx: number, secIdx: number, slideIdx: number) => {
    const songs = liveSongs
    if (!songs[songIdx]?.sections[secIdx]) return
    const section = songs[songIdx].sections[secIdx]
    const clampedSlide = Math.max(0, Math.min(slideIdx, section.slides.length - 1))
  ...
  }, [liveSongs, currentSongIndex, currentSectionIndex, currentSlideIndex, goToSlide])

  const prevSlide = useCallback(() => {
    if (currentSlideIndex > 0) {
      goToSlide(currentSongIndex, currentSectionIndex, currentSlideIndex - 1)
    } else if (currentSectionIndex > 0) {
      const prevSection = liveSongs[currentSongIndex].sections[currentSectionIndex - 1]
      goToSlide(currentSongIndex, currentSectionIndex - 1, prevSection.slides.length - 1)
    } else if (currentSongIndex > 0) {
      const prevSong = liveSongs[currentSongIndex - 1]
      const lastSec = prevSong.sections[prevSong.sections.length - 1]
      goToSlide(currentSongIndex - 1, prevSong.sections.length - 1, lastSec.slides.length - 1)
    }
  }, [liveSongs, currentSongIndex, currentSectionIndex, currentSlideIndex, goToSlide])
  ...
  const jumpToChorus = useCallback(() => {
    const song = liveSongs[currentSongIndex]
    if (!song) return
    const chorusIdx = song.sections.findIndex(s =>
      s.type === 'chorus' || s.label.toLowerCase().includes('chorus')
    )
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      switch (e.key) {
        ...
        case 'l': case 'L': toggleLogo(); break
        case 'r': case 'R': jumpToChorus(); break
  ...
  }, [nextSlide, prevSlide, toggleBlank, toggleLogo, jumpToChorus, goToSlide, liveSongs])
  ...
  const SECTION_COLORS: Record<string, string> = {
    verse: '#4d8ef0', chorus: '#3ecf8e', bridge: '#f5a623',
    'pre-chorus': '#9f7aea', outro: '#888', intro: '#888',
    tag: '#f05252', interlude: '#888'
  }
    ...
    return (
      <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ textAlign: 'center', color: 'var(--text-muted)', fontSize: 13 }}>
          ...
          <div style={{ fontSize: 11 }}>Go to Planner → select a Sunday → Open in builder → Go live</div>
        </div>
      </div>
    )
  }
    ...
    <div style={{ display: 'flex', height: '100%', overflow: 'hidden' }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{
            ...
            background: projectionOpen ? 'var(--accent-green)' : 'var(--border-default)',
          ...
          {!projectionOpen && (
            <button className="btn btn-success" style={{ fontSize: 10, padding: '2px 8px', marginLeft: 'auto' }}
            ...
            <div key={song.lineupItemId} style={{ marginBottom: 8 }}>
              ...
              <div
                ...
                style={{
                  ...
                  background: currentSongIndex === songIdx ? 'var(--accent-blue-dim)' : 'var(--surface-2)'
                }}
              >
                <div style={{
                  fontSize: 11, fontWeight: 600,
                  color: currentSongIndex === songIdx ? 'var(--accent-blue)' : 'var(--text-primary)',
              ...
              {currentSongIndex === songIdx && song.sections.map((sec, secIdx) => {
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      background: isCurrentSec ? 'var(--accent-blue-dim)' : 'var(--surface-1)',
                  ...
                  >
                    <div style={{
                      ...
                      {sec.label}
                      {isCurrentSec && (
                        <span style={{ marginLeft: 5, fontSize: 8, color: 'var(--accent-green)' }}>
                          ● LIVE
                        </span>
                      )}
                    </div>
                    <div style={{
                      fontSize: 10, color: isCurrentSec ? 'var(--accent-blue)' : 'var(--text-secondary)',
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, padding: 16, display: 'flex', flexDirection: 'column', gap: 10, overflow: 'hidden' }}>
          ...
          <div style={{
            flex: 1, background: '#03030a', borderRadius: 10,
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            flexDirection: 'column', gap: 6, padding: 24, position: 'relative',
            ...
            {isBlank && (
              <div style={{
                ...
                <span style={{ color: 'rgba(255,255,255,0.15)', fontSize: 13 }}>Screen blank</span>
              </div>
            )}
            {isLogo && (
              <div style={{
                position: 'absolute', inset: 0, background: '#000', borderRadius: 10,
                display: 'flex', alignItems: 'center', justifyContent: 'center'
              }}>
                <span style={{ color: 'rgba(255,255,255,0.15)', fontSize: 24, fontWeight: 700 }}>
            ...
            {!isBlank && !isLogo && currentSlide && (
              <>
                ...
                <div style={{ color: 'rgba(255,255,255,0.25)', fontSize: 10, marginTop: 8 }}>
                  {currentSong?.title} · {currentSection?.label} · {currentSlideIndex + 1}/{currentSection?.slides.length}
                </div>
              </>
            )}
          </div>
          ...
          {currentSlide && (
            <div style={{
              background: 'var(--surface-2)', borderRadius: 8, padding: '8px 14px',
              border: '1px solid var(--border-subtle)', flexShrink: 0
            }}>
              <div className="label" style={{ marginBottom: 4 }}>Next slide</div>
        ...
        <div style={{
          ...
          <button className="btn" style={{ fontSize: 12 }} onClick={jumpToChorus}>
            ↩ Chorus
          </button>

          <div style={{ flex: 1 }} />

          <button className="btn" style={{ fontSize: 13, padding: '5px 14px' }} onClick={prevSlide}>
            ‹ Prev
          </button>
          <button className="btn btn-primary" style={{ fontSize: 13, padding: '5px 18px', fontWeight: 600 }} onClick={nextSlide}>
            Next ›
          </button>
        </div>
      </div>
      ...
      <div style={{
        width: 160, flexShrink: 0, borderLeft: '1px solid var(--border-subtle)',
        ...
        <div style={{ borderTop: '1px solid var(--border-subtle)', paddingTop: 10 }}>
          <div className="label" style={{ marginBottom: 8 }}>Stream Deck</div>
          {[
            ...
            <div key={btn.label} style={{
              ...
              fontSize: 10, fontWeight: 600, color: btn.color
            }}>
              {btn.label}
            </div>
          ))}
        ...
        <div style={{ marginTop: 'auto' }}>
          <button className="btn btn-danger" style={{ width: '100%', fontSize: 11 }} onClick={onClose}>
            End service
          </button>
        </div>
      </div>
